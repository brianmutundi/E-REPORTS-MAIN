 ...
 * the chosen template form and the term period (opening + closing dates) that
 * appears on generated reports. Reuses `report_template_configs` for the dates
 * and `report_templates.template_key` for the form selection, so generated
 * reports, the screen preview and the PDF route all read from one source.
 * Rejects a closing date that is earlier than the opening date.
 */
