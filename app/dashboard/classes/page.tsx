...
const CLASS_ERR = {
  ...
  emptyName: encodeURIComponent('Error|Class name is required.'),
...
export default async function ClassesPage({ searchParams }: { searchParams: Promise<{ edit?: string; saved?: string; deleted?: string; error?: string }> }) {
  ...
  return <main className="main" style={{ maxWidth: 1050, margin: '0 auto' }}><div className="top"><div><div className="eyebrow">Academic setup</div><h1 className="title">Classes</h1><p className="muted">Manage the classes used for examination reports.</p></div><Link className="btn secondary" href="/dashboard/students">Students</Link></div>
    {params.error && (() => { const idx = params.error.indexOf('|'); return idx > -1 ? <div className="notice error"><span className="font-semibold">{params.error.slice(0, idx)}</span><span className="block text-xs opacity-80 mt-0.5">{params.error.slice(idx + 1)}</span></div> : <div className="notice error">{params.error}</div> })()}{params.saved && <SuccessToast message="Class saved" />}{params.deleted && <SuccessToast message="Class deleted" />}
