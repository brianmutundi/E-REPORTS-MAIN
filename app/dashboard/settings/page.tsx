...
async function createAcademicYear(formData: FormData) {
  ...
  if (!Number.isInteger(sourceYear) || !Number.isInteger(targetYear) || targetYear <= sourceYear) {
    redirect('/dashboard/settings?error=' + encodeURIComponent('Academic year error|The target year must be later than the source year.'))
  }
...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{saved?:string;error?:string;year_created?:string;promoted?:string;terminal?:string;skipped?:string;existing?:string}> }) {
  ...
  const { data: yearRows } = await supabase
    ...
    .limit(10)
  const latestYear = yearRows?.[0]?.year ?? new Date().getFullYear()
  ...
  return <main className="main" style={{maxWidth:850,margin:'0 auto'}}>
    ...
    <div className="card" style={{display:'grid',gap:15,marginTop:24}}>
      <div>
        <h2 className="section-heading" style={{margin:0,fontSize:18}}>Academic years</h2>
        <p className="muted" style={{marginTop:4}}>Create the next academic year and automatically progress active learner placements from each grade to the next, up to Grade 9.</p>
      </div>
      <form action={createAcademicYear} style={{display:'grid',gap:15}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
          <label className="field-label">Source academic year<input name="source_year" type="number" min="2000" max="2100" defaultValue={latestYear} required/></label>
      ...
      {(yearRows?.length ?? 0) > 0 && <div style={{display:'grid',gap:6}}>
        ...
        {yearRows?.map((row) => <div key={row.year} style={{display:'flex',justifyContent:'space-between',padding:'8px 10px',border:'1px solid var(--border,#e2e8f0)',borderRadius:8}}><span>{row.year}</span><span className="muted">{row.status}</span></div>)}
      </div>}
    </div>
