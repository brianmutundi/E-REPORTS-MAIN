  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          <p className="eyebrow">Assessment Broadsheet</p>
          <h1 className="title">Broadsheet</h1>
          <p className="muted mt-1">
            Broadsheets list learners by name with sequential numbering. Ranking and position are used only in Assessment Analysis.
          </p>
        </div>

        <div className="flex items-center gap-2.5 self-start sm:self-auto">
          <Link href="/dashboard/marks" className="btn secondary">Assessment Scores</Link>
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
        <CascadingFilterBar
          basePath="/dashboard/broadsheets"
          submitLabel="View Broadsheet"
      ...
      {selected ? (
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden space-y-6 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
            <div>
              <Link
                href={`/dashboard/broadsheets?exam=${params.exam}&class=${params.class}${params.stream ? `&stream=${params.stream}` : ''}`}
                ...
                {results.length ? results.map((r, index) => (
                  <tr key={r.studentId} className="group hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="sticky left-36 z-10 bg-white group-hover:bg-slate-50/80 transition-colors px-4 py-3.5">
                      <Link href={`/dashboard/broadsheets?exam=${params.exam}&class=${params.class}${params.stream ? `&stream=${params.stream}` : ''}&student=${r.studentId}`} className="font-medium text-slate-900 hover:text-emerald-700 transition-colors">{r.fullName}</Link>
                    </td>
